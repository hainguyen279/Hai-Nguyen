  ...
  function startPlayback(){
    // Bật tiếng ngay trong cử chỉ chạm/click, không để lọt ra ngoài user-gesture
    video.muted = false;
    video.volume = volume.value || 0.75;

    const p = video.play();
    if (p && typeof p.then === 'function') {
      p.catch(err => {
        // Nếu trình duyệt từ chối phát có tiếng, thử lại 1 lần:
        // pause rồi play lại trong cùng gesture (khắc phục bug iOS Safari)
        video.pause();
        video.muted = false;
        video.play().catch(err2 => {
          statusText.textContent = 'Không phát được âm thanh: ' + (err2.name || err2.message || 'lỗi không rõ');
          console.error('Playback blocked:', err2);
        });
      });
    }
  }
  ...
  // click cho desktop, touchend cho mobile để phản hồi tức thì trong user-gesture
  gateBtn.addEventListener('click', unlockAndPlay);
  gateBtn.addEventListener('touchend', (e)=>{ e.preventDefault(); unlockAndPlay(); }, {passive:false});
